...
{
    int sum = 0;
    int lenght = 0; //эта переменная задается для того, чтобы присваивать ей значение меньшего измерения (либо строк либо столбцов) 
    if (matr.GetLength(0) > matr.GetLength(1)) lenght = 1;  // присвоение меньшего значения
    {
        for (int i = 0; i < matr.GetLength(lenght); i++)
        {
                if (i == i) sum += matr[i, i];
        }
    }
